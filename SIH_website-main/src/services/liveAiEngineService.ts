...
import { AIEstimateResult, AIEvaluation } from '../types/database';
...
export class LiveAiEngineService {
  ...
  static async lookupUidai(aadhaar: string, customLink?: { name?: string; phone?: string; email?: string; role?: string }): Promise<{
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/uidai-lookup`, {
        method: 'POST',
  ...
  static async estimateProblemStatement(
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/estimate-cost`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  static async evaluateStartupPitchDeck(params: {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/evaluate-startup`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
